...
func start() -> void:
	var test_lambda: Callable = func(p_step, p_target: CharacterBody2D, p_monitor):
	...
	var callback_lambda = func(p_step: int, p_target: CharacterBody2D, p_is_transition: bool, p_monitor: Monitor):
...
func create_character(p_position: Vector2, p_block_on_wall, p_body_shape := PhysicsTest2D.TestCollisionShape.RECTANGLE) -> CharacterBody2D:
	...
	character.floor_max_angle = deg2rad(44)
